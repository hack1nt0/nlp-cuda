...
//        for (int i = 1; i < blockPrefix.capacity; ++i) blockPrefix[i] = op(blockPrefix[i], blockPrefix[i - 1]);
////    blockPrefix.print();
//
...
__global__
void fillKernel(T *target, ETYPE e, int size) {
...
void spDotKernel(T *C, MatrixA A, MatrixB B, int cPadding) {
    ...
    __shared__ int index[MAX_ROW_LENGTH_OF_SPARSE_MATRIX];
    __shared__ int nnz;
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
