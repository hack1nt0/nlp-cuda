...
RcppExport SEXP new_dtm(SEXP trainWordListListSEXP, SEXP testWordListListSEXP, SEXP parSEXP) {
    BEGIN_RCPP
        vector<Document> trainWordListList = RU::toWordListList(trainWordListListSEXP);
        int parallel = as<int>(parSEXP);
        dtm_t dtm(trainWordListList.size(), trainWordListList.data(), parallel);
        if (Rf_isNull(testWordListListSEXP)) {
            return RU::toDtmR(dtm);
        }
        S4 tr("dtm");
        ...
        std::vector<Document> testWordListList = RU::toWordListList(testWordListListSEXP);
        dtm_t test(testWordListList.size(), testWordListList.data(), parallel, &dtm);
        S4 te("dtm");
...
RcppExport SEXP test_dtm(SEXP xSEXP, SEXP ySEXP) {
    BEGIN_RCPP
        cout << NAMED(xSEXP) << endl;
        cout << NAMED(ySEXP) << endl;
    END_RCPP
}

RcppExport SEXP scale_dtm(SEXP dtmSEXP, SEXP LSEXP, SEXP byrowSEXP) { //todo
    BEGIN_RCPP
        sm_t sm = RU::toSparseMatrix(dtmSEXP);
        sm_t sm2 = sm.clone();
        int L = as<int>(LSEXP);
